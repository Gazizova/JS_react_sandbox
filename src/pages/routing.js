...
function Third({ match }) {
  ...
  return (
    <div>
      ...
      <Route path={`${match.path}/:item`} render={({ match }) => <h3>{match.params.item}</h3>} />
    </div>
  );
}
export default function Routing({ match }) {
  return (
    <div>
      ...
      <Route exact path={match.path} render={() => <h2>Select component</h2>} />
      <Route path={`${match.path}/third`} component={Third} />
      <Route path="/:a/:c/:b(\d{2}-\d{2})" render={({ match }) => <h2>{match.params.b}</h2>} />
    </div>
  );
}
